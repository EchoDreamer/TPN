...
def collate_fn_train(batch):
        n_tokens = [len(e["tokens"]) for f in batch for e in f["exemplars"]]
        max_len = max(n_tokens)        
        n_query=[len(e["tokens"]) for f in batch for e in f["test_examples"]]
        max_query=max(n_query)
        exemplar_tokens = []
        exemplar_mask = []
        ...
        for x in [f["test_examples"] for f in batch]:
            query_tokens.append([e["tokens"] + [0] * (max_query - len(e["tokens"])) for e in x])
            query_mask.append([[1.0] * len(e["tokens"]) + [0.0] * (max_query - len(e["tokens"])) for e in x])        
...
def parse_episodes_from_index(input_file, index_file, tokenizer, markers=True, cache=None, eval_single=False, no_processing=False):
    ...
    # Load file
    input_file = json.load(open(input_file))
    # rel_file=json.load(open(rel_file))
